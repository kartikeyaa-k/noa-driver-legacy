...
class LoginRepository {
  ...
  Future<ApiResponse<DriverLogin?>> userLogin(
      String userName, String password, String firebaseToken) async {
    var apiResponse = await _httpService
        .postRequest(ApiConstant.SERVER + ApiConstant.DRIVER_LOGIN, data: {
      "userName": userName,
      "password": password,
    ...
    if (apiResponse.httpCode == 200) {
      ...
      await prefs
          .setString('logininfo', jsonEncode(apiResponse.data.data))
          .then((value) {
  ...
  Future<ApiResponse<dynamic>> postRegistration(
      BodyRegistrationData data) async {
    var apiResponse = await _httpService.postRequest(
        ApiConstant.SERVER + ApiConstant.REGISTRATION_POST,
        data: data.toJson());
  ...
  Future<ApiResponse<List<CountryResponseData?>>> getCountry() async {
    var apiResponse = await _httpService.getRequest(
      ApiConstant.SERVER + ApiConstant.COUNTRY,
    );
  ...
  Future<ApiResponse<List<StateList?>>> getState(int countryID) async {
    var apiResponse = await _httpService.getRequest(
      ApiConstant.SERVER + ApiConstant.STATE + "/${countryID}",
    );
  ...
  Future<ApiResponse<List<CityListResponseData?>>> getCity(int stateId) async {
    var apiResponse = await _httpService.getRequest(
      ApiConstant.SERVER + ApiConstant.CITY + "/%202002",
    );
